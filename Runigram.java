...
public class Runigram 
{
	...
	{
		int reVal = pixel.getRed();
		int greenVal = pixel.getGreen();
		int blueVal = pixel.getBlue();
		int grayVal = (int)(reVal * 0.299 + greenVal * 0.587 + blueVal * 0.114);
		Color newColor = new Color(grayVal);
		return newColor;
	}
	...
	public static Color[][] scaled(Color[][] image, int width, int height) 
	{
		...
		int newI = image.length / height;
		int newJ = image[0].length / width;
		
		for (int i = 0; i < height; i++)
		{
			for (int j = 0; j < width; j++)
			{
				scaledImage[i][j] = image[i * newI][j * newJ];
			}
		}
		return scaledImage;
	}
